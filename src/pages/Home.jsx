...
function TestimonialsCarousel({ temoignages }) {
  ...
  if (temoignages.length === 0) {
    return (
      <div className="max-w-lg mx-auto bg-white/5 border border-white/10 rounded-2xl p-10 text-center">
        <Users className="text-[#C9A227] mx-auto mb-4" size={36} />
        <p className="text-gray-300 text-sm">Nos références clients seront publiées très prochainement.</p>
      </div>
    )
  }
  ...
  return (
    <div className="relative max-w-2xl mx-auto px-10">
      ...
      {temoignages.length > 1 && (
        <>
          <button onClick={() => { setDir(-1); setCurrent(c => (c - 1 + temoignages.length) % temoignages.length) }}
            className="absolute left-0 top-1/2 -translate-y-1/2 w-9 h-9 bg-white/10 hover:bg-[#C9A227] text-white hover:text-[#065280] rounded-full flex items-center justify-center transition-all">
            <ChevronLeft size={18} />
          </button>
          <button onClick={() => { setDir(1); setCurrent(c => (c + 1) % temoignages.length) }}
            className="absolute right-0 top-1/2 -translate-y-1/2 w-9 h-9 bg-white/10 hover:bg-[#C9A227] text-white hover:text-[#065280] rounded-full flex items-center justify-center transition-all">
...
>
      ...
      <section className="bg-[#F4F6F8] py-20 relative overflow-hidden">
        <div className="max-w-7xl mx-auto px-4 relative">
            ...
            {services.map((service, i) => {
              ...
              return (
                ...
                >
                  ...
                  <Link to="/services"
                    className="inline-flex items-center gap-1.5 text-[#0A69AD] font-bold text-sm group-hover:gap-3 transition-all"
                  >
          ...
          >
            <Link to="/services"
              className="inline-flex items-center gap-2 bg-[#065280] hover:bg-[#0A69AD] text-white font-bold px-7 py-3.5 rounded-xl transition-all text-sm hover:scale-105 shadow-lg"
            >
      ...
      <section className="bg-white py-20">
        <div className="max-w-7xl mx-auto px-4">
          ...
          {temoignages.length === 0 ? (
            <div className="bg-[#F4F6F8] border border-gray-100 rounded-2xl p-12 max-w-lg mx-auto text-center">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
              {temoignages.map((ref, i) => (
                ...
                >
                  <div className="h-36 flex items-center justify-center p-6 bg-[#F4F6F8] border-b border-gray-100">
                    {ref.photo ? (
                      <img src={ref.photo} alt={ref.nom} className="max-h-24 max-w-full object-contain group-hover:scale-105 transition-transform duration-300" />
                    ) : (
                  ...
                  <div className="p-5">
                    <h3 className="font-black text-[#065280] text-base mb-3">{ref.nom}</h3>
                    <div className="flex items-center gap-2 mb-3">
                      {ref.secteur && (
                        <span className="bg-[#065280] text-white text-[10px] font-black px-3 py-1 rounded-full uppercase">
                          {ref.secteur}
                        </span>
                      )}
                      {ref.annee && <span className="text-gray-400 text-sm">{ref.annee}</span>}
                    </div>
                    {ref.texte && <p className="text-gray-500 text-xs leading-relaxed">{ref.texte}</p>}
                  </div>
        ...
        <div className="relative max-w-4xl mx-auto px-4 text-center">
          <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }}>
            ...
            <div className="flex flex-col sm:flex-row gap-3 justify-center">
              <Link to="/contact"
                className="inline-flex items-center justify-center gap-2 bg-[#065280] hover:bg-[#0A69AD] text-white font-black px-8 py-4 rounded-xl transition-all shadow-2xl text-sm hover:scale-105"
              >
